...
void Ncol_molecular_sample::generateStorage(Sample_storage * & store)
{
...
void Ncol_molecular_sample::saveUpdate(int e, Sample_storage * store) {
...
void Ncol_molecular_sample::restoreUpdate(int e, Sample_storage * store)
{
...
void Ncol_molecular_sample::updateEIDist()
{
 ...
 */
void Ncol_molecular_sample::updateEEDist()
{
...
void Ncol_molecular_sample::init(System * sys) {
  cout << "Ncol_molecular_sample::init" << endl;
  assert(sys != NULL);
...
void Ncol_molecular_sample::randomGuess()
{
...
void Ncol_molecular_sample::setElectronPos(const int e,
...
void Ncol_molecular_sample::setElectronPosNoNotify(const int e, 
                                    const Array1 <doublevar> & position) {
  assert( position.GetDim(0) == 3 );
  Array1 <doublevar> temp(position.GetDim(0));
  temp=position;
...
void Ncol_molecular_sample::getElectronPos(const int e, Array1 <doublevar> & R){
...
void Ncol_molecular_sample::moveIon(const int ion, const Array1 <doublevar> & r)
{
  assert(r.GetDim(0) >=3);
...
void Ncol_molecular_sample::rawOutput(ostream & os)
{
  os << "Ncol_molecular_sample\n";
...
void Ncol_molecular_sample::rawInput(istream & is)
{
  string text;
  int nions;
  is >> text;
  if(text != "Ncol_molecular_sample")
    error("expected Ncol_molecular_sample, got ", text);
