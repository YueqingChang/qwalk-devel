...
#include "Ncol_molecular_system.h"
#include "Array.h"
#include "Wavefunction.h"
#include "Sample_point.h"
#include "Ncol_molecular_sample.h"
#include "qmc_io.h"

void Ncol_molecular_system::notify(change_type change, int n)
{
  switch(change) {
  default:
...
int Ncol_molecular_system::generateSample(Sample_point *& samptr)
{
  assert(samptr==NULL);
  samptr=new Ncol_molecular_sample;
  cout << "state init sample" << endl;
  samptr->init(this);
  cout << "finish init sample" << endl;
 
  return 1;
}

int Ncol_molecular_system::showinfo(ostream & os)
{
  ions.showinfo(os);
  return 1;
}

int Ncol_molecular_system::read(vector <string> & words,
                           unsigned int & pos)
{
  nspin.Resize(2);
  ...
  if(!readsection(words, pos, spintxt, "NSPIN")) {
    error("Need NSPIN in noncollinear_molecular system");
  }
...
void Ncol_molecular_system::setIonPos(int ion, Array1 <doublevar> & r)
{
  assert(r.GetDim(0) >=3);
...
void Ncol_molecular_system::calcLocWithTestPos(Sample_point * sample,
...
doublevar Ncol_molecular_system::calcLoc(Sample_point * sample) {
  int nions=sample->ionSize();
  int nelectrons=sample->electronSize();
...
void Ncol_molecular_system::calcLocSeparated(Sample_point * sample, Array1<doublevar> & PotLoc)
{
...
void Ncol_molecular_system::separatedLocal(Sample_point * sample,Array1 <doublevar> & onebody,
    Array2 <doublevar> & twobody)
{
  int nions=sample->ionSize();
  int nelectrons=sample->electronSize();
...
void Ncol_molecular_system::locDerivative(int ion, Sample_point * sample,
                                     Force_fitter & fitter,
...
int Ncol_molecular_system::getBounds(Array2 <doublevar> & latticevec,Array1<doublevar> & origin) { 
  cout << "getBounds()" << endl;
