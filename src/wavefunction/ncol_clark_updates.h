...
*/
#ifndef NCOL_CLARK_UPDATES_H_INCLUDED
#define NCOL_CLARK_UPDATES_H_INCLUDED
#include "Array.h"
...
class ncol_Excitation_list { 
  public:
     void build_excitation_list(Array3 <Array1 <int> > & occupation,int f);//(function,det,spin) (orb #) )
     template <class T> void ncol_clark_updates(Array2 <T> & ginv, Array2 <T> & M,
         int s, Array1 <T> & ratios);
  private:
     Array1 <ncol_Excitation> excitations;
     Array1 <ncol_Excitation> remap;
     vector <vector <int> > allg,alle;
};

//---------

template <class T> void ncol_Excitation_list::ncol_clark_updates(Array2 <T> & ginv, Array2 <T> & M,
     int s, Array1 <T> & ratios) { 
  int ne=M.GetDim(0);
